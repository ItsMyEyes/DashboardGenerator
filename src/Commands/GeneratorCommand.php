...
use KiyoraDashboard\Models\ModelGenerator;
...
abstract class GeneratorCommand extends Command
{
     ...
     * @var string
     */
    protected $modelNamespace = 'KiyoraDashboard\Models';
     ...
     * @var string
     */
    protected $controllerNamespace = 'KiyoraDashboard\Http\Controllers';
     ...
     * @var string
     */
    protected $layout = 'layouts.admin_lte';
...
By ItsMyEyes
     ...
     *
     */
    protected function getDist($type, $content = true)
    {
        $dist = config('crud.dist', 'default');
        if ($dist == 'default') {
            $dist = __DIR__ . '/../../Stub/dist';
        }

        $path = Str::finish($dist, '/') . "{$type}";
     ...
     * @return string
     */
    private function _getLayoutNavPath()
    {
        return $this->makeDirectory(resource_path("/views/layouts/nav.blade.php"));
    }
    ...
    {
        $this->info('Infoing Layout ...');
        if (!(view()->exists($this->layout))) {

            $this->info('Creating Layout ...');

            if (in_array($this->layout, array('adminlte', 'deskapp'))) {
                if ($this->files->exists(public_path('vendor'))) {
                    $this->info('Directory already exists and deleting...');
                    $this->files->deleteDirectories(public_path('vendor'));
                }
                $this->files->copy($this->getStub('layouts/' . $this->layout . '/app', false), $this->_getLayoutPath());
                $this->files->copy($this->getStub('layouts/' . $this->layout . '/nav', false), $this->_getLayoutNavPath());
                $this->files->copyDirectory($this->getDist($this->layout, false), public_path('vendor'));
                $this->info('Creating new layout Done');
            } else {
